...
- `VERSIONING.md`：正式发版、tag、GitHub Release 流程
...
- 开发流程虽然已有文档基础，但“任务生命周期 / 进度同步 / 完成定义”此前仍偏分散，接手时还有靠记忆补齐的风险
...
这轮不再新增产品能力，而是先把“下一位维护者如何稳定接手、推进、收口”标准化下来。

当前要补的不是更多脚本，而是统一口径：

- 明确 `roadmap / workflow / contributing / versioning / changelog` 各自负责什么
- 把“接手 -> 设计 -> 实现 -> 收口 -> 同步 -> 交接”固定成标准任务生命周期
- 把 `roadmap` 继续作为当前状态快照与下一轮起手点，而不是让 release、commit 和记忆各说各话
- 把 installed-skill 同步、文档闭环、完成定义收进默认收口动作，而不是留到最后临时补

这一项做完后，后续再推进真实样本校准、章节退化路径或 compare / dashboard 迭代时，接手成本会明显下降。
