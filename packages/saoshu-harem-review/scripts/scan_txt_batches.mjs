...
import { readNovelWithChapterDetection } from "./lib/novel_input.mjs";
import {
  DEPRESSION_RULES,
...
function parseArgs(argv) {
  ...
  if (!["script", "assist", "auto"].includes(String(out.chapterDetectMode))) scriptUsage("`--chapter-detect-mode` 非法", "允许值：script|assist|auto");
...
function extractTopCharacters(text, topN = 16) {
  ...
  for (const re of patterns) {
    for (const match of text.matchAll(re)) {
      ...
      if (!name || name.length < 2 || name.length > 3) continue;
      if (ignore.has(name)) continue;
      if (!surname.includes(name[0])) continue;
      if (bad.some((item) => name.includes(item))) continue;
      if (badTail.has(name[name.length - 1])) continue;
...
function detectHits(batchText, batchRangeText, rules) {
  ...
  for (const rule of rules.thunderStrict) {
    let matched = "";
    for (const pattern of rule.patterns) {
      if (batchText.includes(pattern)) {
        matched = pattern;
        break;
      }
    }
    if (matched) {
      thunder.push({ rule: rule.rule, summary: `高特异词命中：${matched}（自动预扫，需人工复核）`, evidence_level: "未知待证", anchor: batchRangeText });
    }
  }

  for (const rule of rules.thunderRisk) {
    let matched = "";
    for (const pattern of rule.patterns) {
      if (batchText.includes(pattern)) {
        if (FEMALE_CONTEXT_REQUIRED_RULES.has(rule.rule) && !hasKeywordInContext(batchText, pattern, FEMALE_CONTEXT_WORDS)) continue;
        matched = pattern;
        break;
      }
    }
    if (matched) {
      risks.push({ risk: rule.rule, current_evidence: `检测到风险词 ${matched}`, missing_evidence: "需要具体情节上下文确认是否构成该雷点", impact: "若实锤将显著下调结论并可能直接劝退" });
    }
  }

  for (const rule of rules.depressionRules) {
    const counts = rule.patterns.map((pattern) => ({ pattern, count: countMatches(batchText, pattern) }));
    const total = counts.reduce((sum, item) => sum + item.count, 0);
    const top = counts.sort((a, b) => b.count - a.count)[0];
    if (total >= (rule.minCount || 1) && top && top.count > 0) {
      depression.push({ rule: rule.rule, summary: `关键词命中：${top.pattern} x${top.count}（自动预扫，需人工复核）`, severity: rule.severity, min_defense: rule.min_defense, evidence_level: "未知待证", anchor: batchRangeText });
    }
  }
  ...
  for (const chapter of batch) {
    ...
    for (const rule of titleSignalRules) {
      for (const pattern of rule.patterns) {
        ...
        hits.push({ chapter_num: chapter.num, chapter_title: title, type: rule.type, rule: rule.rule, matched: pattern, weight: rule.weight, critical: rule.critical });
        break;
      }
    }
  }
  hits.sort((a, b) => (b.weight - a.weight) || (a.chapter_num - b.chapter_num));
  return {
...
function loadRuleCatalog(extraRulePath) {
  const catalog = { thunderStrict: THUNDER_STRICT, thunderRisk: THUNDER_RISK, depressionRules: DEPRESSION_RULES, titleSignalRules: TITLE_SIGNAL_RULES };
...
function main() {
  ...
  const loaded = readNovelWithChapterDetection(input, {
  ...
  for (const fileName of fs.readdirSync(output)) {
    if (/^B\d+\.json$/i.test(fileName)) fs.unlinkSync(path.join(output, fileName));
  }
  ...
  for (let start = 0; start < chapters.length; start += stride) {
    ...
    const topSignals = [
      ...hits.thunder.map((item) => ({ name: `雷点:${item.rule}`, count: 1 })),
      ...hits.depression.map((item) => ({ name: `郁闷:${item.rule}`, count: 1 })),
      ...hits.risks.map((item) => ({ name: `风险:${item.risk}`, count: 1 })),
      ...eventCandidates.slice(0, 8).map((item) => ({ name: `事件:${item.rule_candidate}:${item.status}`, count: 1 })),
    ];
    ...
    const payload = {
      ...
      metadata: {
        ...
        chapter_detect: loaded.chapterDetect || {},
      },
      thunder_hits: uniqueBy(hits.thunder, (item) => `${item.rule}|${item.anchor}`),
      depression_hits: uniqueBy(hits.depression, (item) => `${item.rule}|${item.anchor}`),
      risk_unconfirmed: uniqueBy(hits.risks, (item) => `${item.risk}|${item.current_evidence}`),
