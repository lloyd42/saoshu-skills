...
for (const item of cases) {
  ...
  writeJson(perfPath, makeModeDiffFixtureReport({ title: item.title, author: item.author, tags: item.tags, pipelineMode: "performance", batchIds: ["B01", "B02", "B03", "B04"], totalBatches: 4, rating: 6, eventCount: 2, relationCount: 1 }));
  writeJson(econPath, makeModeDiffFixtureReport({ title: item.title, author: item.author, tags: item.tags, pipelineMode: "economy", batchIds: ["B01", "B02", "B03"], totalBatches: 4, rating: 5, eventCount: 1, relationCount: 0 }));
...
if (!hasFailures()) console.log("Mode diff ledger check passed.");
