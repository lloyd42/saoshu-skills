...
function usage() {
  console.log("Usage:");
  console.log("  node manifest_wizard.mjs --output <manifest.json> [--preset newbie|full] [--coverage-mode sampled|chapter-full|full-book]");
  console.log("  node manifest_wizard.mjs --output <manifest.json> --non-interactive --input-txt <txt> --output-dir <dir> --title <name> [--author <name>] [--coverage-mode sampled|chapter-full|full-book] [--coverage-template opening-100|head-tail|head-tail-risk|opening-latest] [--serial-status unknown|ongoing|completed]");
  console.log("");
  console.log("Notes:");
  console.log("  - preset=newbie 默认生成“快速摸底”配置：coverage_mode=sampled（兼容 economy） + dynamic + auto + local db");
  console.log("  - preset=full 默认生成“章节级尽量完整”配置：coverage_mode=chapter-full（兼容 performance） + local db");
  console.log("  - 如需“整书最终确认”，可显式传 --coverage-mode full-book；wizard 会自动写入兼容的 pipeline_mode");
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (key === "--coverage-mode") out.coverageMode = value, i++;
    else if (key === "--coverage-template") out.coverageTemplate = value, i++;
    else if (key === "--serial-status") out.serialStatus = value, i++;
...
function applyCoverageModeDefaults(manifest, coverageMode) {
  manifest.coverage_mode = coverageMode;
  manifest.pipeline_mode = pipelineModeForCoverageMode(coverageMode);
  if (coverageMode === "sampled") {
    ...
    manifest.sample_level = "high";
    if (!(Number(manifest.sample_count) > 0)) manifest.sample_count = 7;
  }
}

function syncCoverageCompatibility(manifest) {
  if (!manifest.coverage_mode) return;
  manifest.pipeline_mode = pipelineModeForCoverageMode(manifest.coverage_mode);
}

function applyPreset(manifest, preset) {
  if (preset === "newbie") {
    applyCoverageModeDefaults(manifest, "sampled");
    manifest.db_mode = "local";
    manifest.enrich_mode = "fallback";
    manifest.report_default_view = "newbie";
    manifest.chapter_detect_mode = "auto";
  } else {
    applyCoverageModeDefaults(manifest, "chapter-full");
    manifest.sample_mode = "fixed";
...
async function askOptionalChoice(rl, question, allowed, fallback = "") {
  const value = await ask(rl, `${question}（可留空；${allowed.join("/")}）`, fallback);
  if (!value) return "";
  assertChoice(value, allowed, question);
  return value;
}
...
function validateManifest(manifest, options = {}) {
  ...
  assertChoice(manifest.coverage_mode, COVERAGE_MODES, "coverage_mode");
  assertChoice(manifest.pipeline_mode, PIPELINE_MODES, "pipeline_mode");
  if (manifest.pipeline_mode !== pipelineModeForCoverageMode(manifest.coverage_mode)) throw new Error("coverage_mode 与 pipeline_mode 不一致");
  ...
  if (manifest.coverage_template) {
    assertChoice(manifest.coverage_template, COVERAGE_TEMPLATES, "coverage_template");
    if (manifest.coverage_mode !== "sampled") throw new Error("coverage_template 仅能与 sampled 搭配");
  }
...
async function interactiveFill(manifest) {
  ...
  try {
    ...
    manifest.coverage_mode = await askChoice(rl, "覆盖模式（sampled=快速摸底 / chapter-full=章节级尽量完整 / full-book=整书最终确认）", COVERAGE_MODES, manifest.coverage_mode);
    applyCoverageModeDefaults(manifest, manifest.coverage_mode);
    if (manifest.coverage_mode === "sampled") manifest.coverage_template = await askOptionalChoice(rl, "快速摸底模板", COVERAGE_TEMPLATES, manifest.coverage_template);
    else manifest.coverage_template = "";
    ...
    if (manifest.coverage_mode === "sampled") {
...
function main() {
  ...
  if (args.coverageMode) applyCoverageModeDefaults(manifest, args.coverageMode);
  if (args.coverageTemplate !== undefined) manifest.coverage_template = args.coverageTemplate;
  if (args.serialStatus) manifest.serial_status = args.serialStatus;

  const finish = () => {
    syncCoverageCompatibility(manifest);
