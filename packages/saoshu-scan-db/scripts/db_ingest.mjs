...
function main() {
  ...
  const run = {
    ...
    reader_policy_source: report.reader_policy?.source || "",
    ...
    reader_policy_hard_blocks: arr(report.reader_policy?.hard_blocks).map((item) => String(item || "").trim()).filter(Boolean),
    reader_policy_soft_risks: arr(report.reader_policy?.soft_risks).map((item) => String(item || "").trim()).filter(Boolean),
    reader_policy_relation_constraints: arr(report.reader_policy?.relation_constraints).map((item) => String(item || "").trim()).filter(Boolean),
    ...
    selected_batches: Number(report.scan?.sampling?.selected_batches || 0),
