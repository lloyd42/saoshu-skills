...
<div class="card"><h3>最近运行</h3><table><thead><tr><th>时间</th><th>标题</th><th>结论</th><th>评分</th><th>覆盖口径</th><th>模板</th><th>兼容执行层</th><th>覆盖单元</th><th>识别路径</th><th>状态</th><th>覆盖率</th><th>未覆盖提醒</th></tr></thead><tbody>${latest.map((r)=>`<tr><td>${esc(r.ingested_at || "")}</td><td>${esc(r.title || "")}</td><td>${esc(r.verdict || "")}</td><td>${esc(r.rating || "")}</td><td>${esc(r.coverage_mode || "")}</td><td>${esc(r.coverage_template || "")}</td><td>${esc(r.pipeline_mode || "")}</td><td>${esc(r.coverage_unit || "")}</td><td>${esc(r.chapter_detect_used_mode || "")}</td><td>${esc(r.serial_status || "")}</td><td>${Number.isFinite(Number(r.coverage_ratio)) ? `${(Number(r.coverage_ratio) * 100).toFixed(1)}%` : "-"}</td><td>${esc(r.coverage_gap_summary || "-")}</td></tr>`).join("") || "<tr><td colspan=12>-</td></tr>"}</tbody></table></div>
