...
<div class="card"><h3>趋势报告入口</h3><div class="muted">按日、作者、标签、读者策略与 mode-diff 的长期分布会落到独立趋势页。</div>${trendsTarget.available ? `<div><a class="linkbtn" href="${esc(trendsTarget.href)}" target="_blank" rel="noopener">打开 trends 详情</a></div>${trendsTarget.generationError ? `<div class="muted" style="margin-top:8px">${esc(`本次自动刷新失败，先沿用已有 trends 页：${trendsTarget.generationError}`)}</div>` : ""}` : `<div class="muted" style="margin-top:8px">${esc(trendsTarget.generationError ? `趋势页自动生成失败：${trendsTarget.generationError}` : "趋势页尚未生成，先执行下面命令。")}</div><code class="cmd">${esc(trendsTarget.command)}</code>`}</div>
